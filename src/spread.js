...
export async function main(ns) {
    ...
    const crackHost = (node) => {
      ns.print(`Host ${node.hostname} requires ${node.requiredHackingSkill} to crack`)
  
      ns.print(`Skill required: ${node.requiredHackingSkill}`)
      if (node.requiredHackingSkill > PLAYER_SKILL) {
        ns.print("Would not crack!")
        return false
      }
  
      if (node.numOpenPortsRequired > CRACKERS.length - 1) {
        ns.print(`Not enough crackers available, requires ${node.numOpenPortsRequired}`)
        return false
      }
    ...
    ns.scan(CURRENT_NODE)
      ...
      .forEach(node => {
        ...
        if (!ns.hasRootAccess(node.hostname)) {
          ...
          ns.print(`${node.hostname} was already cracked.`)
  
        ns.print(`Copying ${SCRIPT_NAME} from ${CURRENT_NODE} to ${node.hostname}`)
    ...
    const availableRam = currentNode.maxRam - (currentNode.ramUsed - SCRIPT_SELF_COST)
